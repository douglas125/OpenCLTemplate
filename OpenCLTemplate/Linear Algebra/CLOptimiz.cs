...
namespace OpenCLTemplate.LinearAlgebra
{
    ...
    public partial class floatLinalg
    {
        ...
        public static class BLAS
        {
            ...
            /// <param name="B">Matrix B</param>
            /// <param name="ans">Answer. If null, gets created.</param>
            public static void MatrMatrProd(floatMatrix A, floatMatrix B, ref floatMatrix ans)
            {
                if (A.Cols != B.Rows) throw new Exception("Incompatible dimensions");
                if (ans == null) ans = new floatMatrix(new float[A.Rows, B.Cols]);
                if (ans.Rows != A.Rows || ans.Cols != B.Cols) throw new Exception("Invalid ans dimensions");

                if (UseOpenCLIfAvailable && CLCalc.CLAcceleration == CLCalc.CLAccelerationType.UsingCL)
                {
                    kernelRegularMatrMatrProd.Execute(new CLCalc.Program.MemoryObject[] { A.CLValues, B.CLValues, ans.CLValues, A.CLDim }, new int[] { A.Rows, B.Cols });
                }
                else
                {
                    int n = A.Cols;
                    ...
                    for (int i = 0; i < A.Rows; i++)
                    {
                        for (int j = 0; j < B.Cols; j++)
                        {
                            ...
                            {
                                int pk = p * k;
                                temp += A.Values[k + ni] * B.Values[j + pk];
                            }
            ...
            public floatVector LinearSolve(floatVector CLbb, bool refine, ref floatVector resp)
            {
                ...
                for (int iter = 0; iter < 8 && !double.IsNaN(totalRes); iter++)
                {
                    ...
                    if (totalRes < 1E-5)
                        iter = 8;
                    {
                        if (UseOpenCLIfAvailable && CLCalc.CLAcceleration == CLCalc.CLAccelerationType.UsingCL) vecResidues.ReadFromDevice();
                        linsolve(vecResidues.Values, ref vecDeltax);
                        if (UseOpenCLIfAvailable && CLCalc.CLAcceleration == CLCalc.CLAccelerationType.UsingCL) vecDeltax.CLValues.WriteToDevice(vecDeltax.Values);
        ...
        public class floatVector
        {
            ...
            {
                if (v.Length != Values.Length) throw new Exception("Incompatible dimensions");
                for (int k = 0; k < Values.Length; k++) Values[k] = v[k];

                if (UseOpenCLIfAvailable && CLCalc.CLAcceleration == CLCalc.CLAccelerationType.UsingCL) CLValues.WriteToDevice(Values);
            }
            #endregion
    ...
    public class floatOptimization
    {
        ...
        public static class CurveFitting
        {
            ...
            public class PNormMin
            {
                ...
                floatLinalg.floatVector CLx;
                floatLinalg.floatMatrix CLA;
                floatLinalg.floatVector CLb;
                floatLinalg.floatVector CLW;
                floatLinalg.floatVector CLlambda;
                floatLinalg.floatVector CLp, CLq;

                PNormMinClass pnm;

                floatLinalg.floatSymPosDefMatrix HessF;
                floatLinalg.floatVector gradF;

                //Line search variables
                floatLinalg.floatVector temp;
                floatLinalg.floatVector deltaX;
                #endregion
                ...
                public PNormMin(int numUnknowns, int numEquations)
                {
                    nUnknowns = numUnknowns;
                    nEqs = numEquations;

                    x0 = new float[numUnknowns];
                    b = new float[numEquations];

                    for (int k = 0; k < numUnknowns; k++) x0[k] = 0.1f + 0.01f * (float)rnd.NextDouble();

                    CLx = new floatLinalg.floatVector(x0);

                    A = new float[numEquations, numUnknowns]; //Ax=b
                    CLA = new floatLinalg.floatMatrix(A);

                    b = new float[numEquations];
                    CLb = new floatLinalg.floatVector(b);

                    W = new float[numEquations];
                    CLW = new floatLinalg.floatVector(W);

                    lambda = new float[numUnknowns];
                    CLlambda = new floatLinalg.floatVector(lambda);

                    CLp = new floatLinalg.floatVector(new float[] { 1 });
                    CLq = new floatLinalg.floatVector(new float[] { 1 });

                    pnm = new PNormMinClass(CLA, CLb, CLW, CLlambda, CLp, CLq);


                    //Creates Hessian and Gradient
                    HessF = new floatLinalg.floatSymPosDefMatrix(x0.Length);
                    gradF = new floatLinalg.floatVector(x0);

                    //Line search variables
                    temp = new floatLinalg.floatVector(x0);
                    deltaX = new floatLinalg.floatVector(x0);
                ...
                public void SetData(float[] x0, float[,] A, float[] b, float[] W, float[] lambda, float p, float q)
                {
                    if (x0.Length != nUnknowns) throw new Exception("Expected x0 Length = " + nUnknowns.ToString());
                    if (b.Length != nEqs) throw new Exception("Expected b Length = " + nEqs.ToString());
                    ...
                    CLx.SetValues(x0);
                    if (A != null) CLA.SetValues(A);
                    CLb.SetValues(b);
                    CLW.SetValues(W);
                    CLlambda.SetValues(lambda);
                    
                    CLp.SetValues(new float[] { p });
                    CLq.SetValues(new float[] { q });
                }
                ...
                {
                    int iters = 0;
                    return UnconstrainedMinimization.Solve(CLx, pnm.F, out iters, HessF, gradF, temp, deltaX, null, null, null, null, null, null, null, null, null, null, null);
                }
            }
            ...
            public static float[] PNormMinimization(float[] x0, floatLinalg.floatMatrix CLA, floatLinalg.floatVector CLb, floatLinalg.floatVector CLW, floatLinalg.floatVector CLlambda, floatLinalg.floatVector CLp, floatLinalg.floatVector CLq)
            {
                float[,] AeqConstr = null;
                float[] bEqConstr = null;
                //Dimensionality check

                if (CLA.Cols != x0.Length) throw new Exception("Number of columns of A should be x.Length");
                if (CLA.Rows != CLb.Length) throw new Exception("Number of rows of A should be b.Length");
                if (CLA.Rows != CLW.Length) throw new Exception("Number of weights should be equal to b.Length");
                if (CLA.Cols != CLlambda.Length) throw new Exception("Number of lambda weights should be equal to x.Length");
                if (CLp.Values[0] < 1 || CLp.Values[0] < 1) throw new Exception("p and q values should be greater than 1");
                if (AeqConstr != null && AeqConstr.GetLength(1) != x0.Length) throw new Exception("Number of columns of AeqConstr should be x.Length");
                if (AeqConstr != null && bEqConstr.Length != AeqConstr.GetLength(0)) throw new Exception("bEqConstr.Length not compatible with AeqConstr");
